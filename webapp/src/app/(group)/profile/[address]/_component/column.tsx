...
type Def = RouterOutputs['userProfile']['getUserNfts'][number];
		...
		cell: ({ row }) => {
			...
			return (
				<div className="font-sans font-thin flex gap-2 items-center">
					...
					<Link
						href={`/collection/${row.original.collection.address}/${row.original.tokenId}`}
						className="whitespace-nowrap hover:underline"
					>
						{formatTokenName(row.original.name, row.original.tokenId)}
					</Link>
				</div>
			);
		},
		...
		cell: ({ row }) => {
			...
			return (
				<div className="flex gap-x-[1px]">
					<span className=" text-lg text-primary text-end ">
						<Currency value={_row.latestActivity?.activity?.price ?? 0} />
					</span>
				</div>
			);
		},
		...
		cell: ({ row }) => {
			...
			return (
				<div className="flex justify-end">
					<Link
						href={`/profile/${_row.latestActivity?.activity?.seller}`}
						className="hover:underline font-thin font-sans"
					>
						{shortenAddress(_row.latestActivity?.activity?.seller || '')}
					</Link>
				</div>
			);
		},
