...
async function withImageURL(metadata: Metadata) {
...
async function fetchTokenMetadata(tokenURI: string) {
  ...
  try {
    ...
    throw new Error(`Error fetch metadata response ${tokenURI}`)
  }
}
...
export const fetchTokens = async (contractAddress: string) => {
  ...
  const tokensMetadata = await Promise.allSettled(
    tokensURI.map(({ tokenId, uri }, i) => {
      return fetchTokenMetadata(uri)
        .then(data => {
          if (i % 3 === 0) {
            SocketHandler.instance.io.emit('token_fetching_progress', {
              address: collection.address,
              progress: (i / totalSupply) * 100,
              name: (collection.metadata as any)?.['name']
            })
          }
          return { tokenId, data }
        })
  ...
  for (const [i, token] of tokensMetadata.entries()) {
